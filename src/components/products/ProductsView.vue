<template>
  <div class="prodd">
    <div
      :class="{
        'products-view-section-grid': !flexView,
        'products-view-section-flex': flexView,
      }"
    >
      <div
        :class="{
          'products-grid-card': !flexView,
          'products-flex-card': flexView,
        }"
      ...
      >
        ...
        <div class="products-flex-card__content">
          <div class="products-flex-card__content-title">
            <h3>{{ product.name }}</h3>
            <span class="ratings-stars">
              <svg
                v-for="el in Math.round(product.rating.value)"
                :key="el"
                width="16"
                height="16"
                viewBox="0 0 16 16"
              ...
              <svg
                v-for="el in 5 - Math.round(product.rating.value)"
                :key="el"
                width="16"
                height="16"
                viewBox="0 0 16 16"
          ...
          <div class="products-flex-card__content-price">
            <p>
              ${{ product.price }} <span>${{ product.wasPrice }}</span>
            </p>
          </div>
          <p>
            {{ product.description }}
    ...
    <div class="products-pagination" v-if="filteredProducts.length">
      <p
        v-for="page in maxPages"
        :key="page"
        :class="{ 'products-pagination-active': page === currentPage + 1 }"
        @click="currentPage = page - 1"
      >
        {{ page }}
      </p>
    </div>
    <div class="products-not-available" v-else>
      <img src="../../assets/basket.png" alt="Empty basket" />
      <h2>Sorry there is no products for your search . . .</h2>
    </div>
  </div>
</template>
...
const products = computed(() => store.getters.getProductsSortedHigh);
  ...
  filteredProducts.value = products.value.filter((product) => {
    return activeFilters.value.length
      ? activeFilters.value.every((filter) => {
          let [prop, val] = filter.split("-");
          if (prop === "discountPercentage") {
            val = +(val[0] + "" + val[1]);
          }
          if (prop === "rating.value") {
            return Math.round(product.rating.value) === +val;
          }
          if (prop === "price") {
            val = [...val.slice(1)].filter((el) => el !== "+").join("");
            console.log(prop, " .... ", val);
            return product.price >= +val && product.price <= +val + 1300;
          }

          console.log(prop, " .... ", val);
          return product[prop] === val;
        })
      : product;
  });

  if (sortHighToLow.value) {
